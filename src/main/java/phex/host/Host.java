...
//    }
    ...
    public boolean isErrorStatusExpired(long currentTime, long expiryDelay) {
        if (status == HostStatus.ERROR || status == HostStatus.DISCONNECTED) {
            if (currentTime - statusTime > expiryDelay) {
                return true;
            }
        }
        return false;
    ...
    public void incSentDropCount(int val) {
        if (val > 0) {// ensure no quality calculation in case nothing was dropped.
    ...
    public void queueMessageToSend(Message message) {
        ...
        if (hopsFlowLimit > -1
                ...
                && message.getHeader().getHopsTaken() >= hopsFlowLimit) {// don't send query!
            return;
        }
