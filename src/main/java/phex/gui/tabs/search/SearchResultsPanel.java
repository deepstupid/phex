 ...
 * 
 *  --- SVN Information ---
 *  $Id: SearchResultsPanel.java 4380 2009-02-21 20:50:52Z gregork $
 */
...
public class SearchResultsPanel extends JPanel
{
     ...
     * representative RemoteFile for this collection is returned, if useRepresent is
    ...
    private void lookupBitziTicket()
    {
        /*
        final RemoteFile rfile = getSelectedRemoteFile( true );
        if ( ! (Float.valueOf( rfile.getBitzRating() ).isNaN()) )
        {
            return;
        }
        Runnable runner = new Runnable()
        {
            public void run()
            {
                ...
                logger.debug( "Lookup Bitzi Ticket for: {}", rfile );
                String sha1 = rfile.getSHA1();
                HttpClient httpClient = HttpClientFactory.createHttpClient();
                String url = "http://ticket.bitzi.com/rdf/" + URLCodecUtils.encodeURL( sha1 );
                ...
                try
                {
                    ...
                    if ( responseCode != 200 )
                    {
                        fileRating = Float.NEGATIVE_INFINITY;
                    }
                    else
                    {
                        InputStream inStream = method.getResponseBodyAsStream();
                        String data = new String( IOUtil.toByteArray( inStream ), "UTF-8" );
                        fileRating = TicketParser.parseBitzFileRating( data );
                    }
                }
                ...
                logger.debug( "Lookup Bitzi Ticket result: {}", fileRating );
                rfile.setBitzRating( fileRating );
            }
        };
        Environment.getInstance().executeOnThreadPool(runner, "LookupBitzi" );
        */
    }
