 ...
 * 
 *  --- SVN Information ---
 *  $Id: NetworkTableModel.java 4377 2009-02-21 20:46:52Z gregork $
 */
...
public class NetworkTableModel extends FWSortableTableModel
{
    ...
    static
    {
        tableColumns = new String[]
        {
            ...
            Localizer.getString( "NetworkTable_RxDropQuality" ),
            Localizer.getString( "NetworkTable_TxQueuedDropQuality" ),
    ...
    public Object getValueAt(int row, int col)
    {
        ...
        switch (col)
        {
            ...
            case RECEIVED_DROPPED_MODEL_INDEX:
                return String.valueOf( host.getReceivedCount() ) + " (" 
                     + String.valueOf( host.getReceivedDropCount() ) + ") "
                     + String.valueOf( Math.round( host.getReceivedQuality()*100 ) ) + "%";

            case SENT_QUEUED_MODEL_INDEX:
                return String.valueOf( host.getSentCount() ) + " ("
                     + String.valueOf( host.getSendQueueLength() ) + " / "
                     + String.valueOf( host.getSentDropCount() ) + ") "
                     + String.valueOf( Math.round( host.getSentQuality()*100 ) ) + "%";
            ...
            case UPTIME_MODEL_INDEX:
                long upSeconds = host.getConnectionUpTime( ) / 1000;
    ...
    public boolean isColumnDefaultVisible( int columnIndex )
    {
        if ( columnIndex == QRT_MODEL_INDEX 
          || columnIndex == SENT_QUEUED_MODEL_INDEX
          || columnIndex == RECEIVED_DROPPED_MODEL_INDEX )
        {
            return false;
        }
        return true;
