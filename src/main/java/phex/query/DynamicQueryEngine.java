 ...
 * 
 *  --- SVN Information ---
 *  $Id: DynamicQueryEngine.java 4318 2008-11-30 22:50:47Z gregork $
 */
package phex.query;
...
public class DynamicQueryEngine implements DynamicQueryConstants {
    ...
    public void processQuery() {
        ...
        if (currentTime < nextProcessTime || isQueryFinished()) {// not our turn to query now...
            return;
        }
    ...
    private void processStandardQuery() {
        ...
        if (notQueriedHosts == 0 || hostToQuery == null) {// no hosts to query found... try again later...
        ...
        if (ttl == 1 && hostToQuery.isUPQueryRoutingSupported()) {// if we have a UP query routing connection we can
    ...
    private void processProbeQuery() {
        ...
        for (Host up : ultrapeers) {
            ...
            if (up.isUPQueryRoutingSupported() && qrt != null) {
                ...
                } else {// add to the end of failed list (low priority)
                    failedList.add(up);
                }
            } else {// add to the top of failed list (high priority)
                failedList.add(0, up);
            }
        }
