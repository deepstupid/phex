 ...
 * 
 *  --- SVN Information ---
 *  $Id: UploadEngine.java 4553 2012-01-20 17:37:24Z gregork $
 */
package phex.upload;
...
public class UploadEngine {
                ...
                try {
                    ...
                    if (!succ) {
                        ...
                        throw new IOException("Status transition from "
                                + uploadState.getStatus() + " to " + UploadStatus.COMPLETED + " failed.");
                    }
                    ...
                    if (!succ) {
                        ...
                        throw new IOException("Status transition from "
                                + uploadState.getStatus() + " to " + UploadStatus.ABORTED + " failed.");
                    }
                    throw exp;
                }
    ...
    private void sendResponseData(UploadResponse response) throws IOException {
        ...
        if (!succ) {
            ...
            throw new IOException("Status transition from "
                    + uploadState.getStatus() + " to " + UploadStatus.UPLOADING_DATA + " failed.");
        }
    ...
    private void readNextHTTPRequest() throws IOException {
        try {
            ...
            if (uploadHandler.isQueued() && System.currentTimeMillis()
                    < uploadHandler.getQueueMinNextPollTime()) {// the request came too soon. Disconnect faulty client...
