 ...
 * 
 *  --- SVN Information ---
 *  $Id: GGEPExtension.java 4530 2011-07-19 09:41:18Z gregork $
 */
package phex.msg;
...
{
    private static final Logger logger = LoggerFactory.getLogger( GGEPExtension.class );
    ...
    public static DestAddress[] parseAltExtensionData( GGEPBlock[] ggepBlocks, PhexSecurityManager securityService )
    {
        ...
        {
            logger.warn( "Invalid ALT GGEPBlock length: {}", 
                HexConverter.toHexString( altLocData ) );
            return null;
        }
    ...
    public static DestAddress[] parsePushProxyExtensionData( GGEPBlock[] ggepBlocks, PhexSecurityManager securityService )
    {
        ...
        if ( data.length % 6 != 0 )
        {
            logger.warn( "Invalid PushProxy GGEPBlock length: {}",
