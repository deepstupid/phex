 ...
 * 
 *  --- SVN Information ---
 *  $Id: QueryResponseMsg.java 4380 2009-02-21 20:50:52Z gregork $
 */
package phex.msg;
...
public class QueryResponseMsg extends Message {
    ...
    private void parseBody()
        ...
        if (offset <= (getHeader().getDataLength() - 16 - 4 - 2)) {
            ...
            if (openDataLength > 1) {   // if we have a flag byte
                ...
                if ((flag1 & GGEP_MASK) != 0 && (flag2 & GGEP_MASK) != 0) {// parse GGEP area should follow after open data area but
                    // we can't be sure...
