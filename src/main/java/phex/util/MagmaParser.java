 ...
 * 
 *  --- SVN Information ---
 *  $Id: MagmaParser.java 4229 2008-07-13 21:42:10Z gregork $
 */
package phex.util;
 ...
 */
    ...
    public void parseMagmaHeader()
        ...
        while (true) {
            ...
            if (c == HEAD_DOWNLOAD_NAME[pos]) {
                ...
                reader.unread(c);
                return;
            } else {// next char of list element not found... search on
                pos = 0;
            }
        }
    }
        ...
        while (true) {
            ...
            if (c == '-') {// we found a list element
                ...
                if (!startOfLine) {
                    ...
                    int pre = reader.read();
                    if (pre == ' ') {// there is a good chance this is a magnet
                        reader.unread(pre);
                        parseListElement();
                    } else {// we are not in a list anymore... this must be the start of a next element.
                        reader.unread(pre);
                        reader.unread(c);
                        return;
                    }
                }
            } else if (c == '#') {// we reached a comment... skip rest of line... (YAML 4.2.3)
                skipToEOL();
                startOfLine = true;
            } else if (c == ' ' || c == '\r' || c == '\n') {// skip leading indetion whitespace..
                startOfLine = false;
            } else if (EOL_CHARACTERS.indexOf(c) != -1) {// skip leading indetion whitespace..
                startOfLine = true;
            } else if (c == -1) {
                // reached the EOF
                return;
            } else {
                if (startOfLine) {// oups this indecates we jumped out of the list body..
                    reader.unread(c);
                    return;
                } else {
    ...
    private void parseListElement()
        ...
        while (true) {
            ...
            if (c == ' ') {// skip leading indetion whitespace..
                continue;
            } else if (c == '#') {// we reached a comment... skip rest of line... (YAML 4.2.3)
                skipToEOL();
            } else if (c == '"') {
    ...
    private void parseMagnet()
        ...
        while (true) {
            ...
            if (c == ' ' || EOL_CHARACTERS.indexOf(c) != -1) {// skip all line folding characters.. and all spaces
                continue;
            } else if (c == '"') {// found the end of the quoted magnet. "
                break;
            } else if (c == -1) {
                // unexpected end...
                return;
    ...
    private void skipToEOL() throws IOException {
        ...
        while (true) {
            ...
            if (c < 0) {// stream ended... a valid line could not be read... return
                return;
            } else if (EOL_CHARACTERS.indexOf(c) != -1) {// we found a line ending... check if there are followups (YAML 4.1.4)
