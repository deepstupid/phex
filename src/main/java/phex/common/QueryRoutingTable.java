 ...
 *
 *  --- SVN Information ---
 *  $Id: QueryRoutingTable.java 4554 2012-01-20 17:38:18Z gregork $
 */
package phex.common;
...
public class QueryRoutingTable {
    ...
    public void updateRouteTable(RouteTableUpdateMsg message, Host sourceHost)
        ...
        if (message.getVariant() == RouteTableUpdateMsg.RESET_TABLE_VARIANT) {
            ...
            if (sequenceSize == 0 && sequenceNumber == 0) {// initializing case...
            ...
            } else if (compressor != QRPatchTableMsg.COMPRESSOR_NONE) {// validation fails..
                throw new InvalidMessageException(
            ...
            if (entryBits == 4) {
                ...
                patchData = buf;
            } else if (entryBits != 8) {// validation fails..
                throw new InvalidMessageException(
    ...
    private BitSet resizeRouteTable(int newSize) {
        ...
        for (int i = 0; i < tableSize; i++) {
            if (!qrTable.get(i)) {// bit not set... don't need to handle...
                continue;
            }
