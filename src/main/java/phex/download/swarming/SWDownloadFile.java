 ...
 * 
 *  --- SVN Information ---
 *  $Id: SWDownloadFile.java 4530 2011-07-19 09:41:18Z gregork $
 */
...
public class SWDownloadFile implements TransferDataProvider, SWDownloadConstants {
    ...
    protected boolean addDownloadCandidate(SWDownloadCandidate candidate) {
        ...
        if ((fileURN != null && candidateURN != null)
                && !fileURN.equals(candidateURN)) {// make sure URNs match!
    ...
    public void onCandidateStatusChange(String topic,
        ...
        if (this != candidate.getDownloadFile()) {// not for me...
            return;
        }
    ...
    public synchronized void moveToDestinationFile() {
        if (isFileCompletedMoved()) {// somebody else did it already before me...
            return;
        }
        ...
        try {
            ...
            if (exp.getCause() instanceof InterruptedException) { // the thread was interrupted and requested to stop, most likley
                // by user request.
    ...
    private void downloadStopNotify() {
        ...
        if (transferStopTime == 0) {
            ...
            if (status == STATUS_FILE_DOWNLOADING) {// only update if the status is currently switched from downloading to stopped..
